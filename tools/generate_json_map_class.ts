...
async function convertFile() {
  ...
  const cfdPath = `${__dirname}/../../${libraryName}/`;
  const cfdPath2 = `${__dirname}/../external/${libraryName}/`;
  let folderPath = `external/cfd-js/src/input_json_format/`;
  ...
  const insertFunctions: TsAppendFunctionData[] = [{
    ...
    returnType: 'void',
    comment: 'Add initialized listener.',
  }, {
    name: 'getCfd',
    parameters: [],
    returnType: 'Cfdjs',
    comment: 'Get cfd-js-wasm object.',
  }, {
    name: 'hasLoadedWasm',
    parameters: [],
    returnType: 'boolean',
    comment: 'Has wasm loading check.',
  }];
  ...
  if (fs.existsSync(cfdPath) && fs.statSync(cfdPath).isDirectory()) {
    ...
    cfdBaseDir = cfdPath2;
    outTsFolderPath = cfdPath2 + outTsFolderPath;
