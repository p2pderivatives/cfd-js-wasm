...
cfdjsWasm['onRuntimeInitialized'] = async () => {
  const ccall = async function(func, returnType, argTypes, args) {
    const module = cfdjsWasm;
  ...
  const callJsonApi = async function(reqName, arg) {
    ...
    try {
      ...
      const retJson = await ccall(cfdjsWasm._cfdjsJsonApi, 'string', ['string', 'string'], [reqName, argStr]);
  ...
  const funcNameResult = await ccall(cfdjsWasm._cfdjsGetJsonApiNames, 'string', [], []);
  ...
  funcList.forEach((requestName) => {
    const hook = async function(...args) {
      ...
      return await callJsonApi(requestName, arg);
    };
